...
int main() {
    ...
    cin >> queryImagePath;
    queryImagePath = "query_image/" + queryImagePath;
    ...
    for (int i = 0; i < dbImages.size(); i++) {
        ...
        vector<KeyPoint> KeypointsQuery, KeypointsDB;
        Mat descriptorsQuery, descriptorsDB;
        orbF->detectAndCompute(query, noArray(), KeypointsQuery, descriptorsQuery);
        orbF->detectAndCompute(image, noArray(), KeypointsDB, descriptorsDB);

        // KNN Matching (k-nearest neighbor matching)
        int k = 2;
        matcher.knnMatch(descriptorsQuery, descriptorsDB, matches, k);
        ...
        if (goodMatches.size() >= 4) {
            ...
            if (score < bestMatchScore) {
                ...
                keypoints1 = KeypointsQuery;
                keypoints2 = KeypointsDB;
                descriptors1 = descriptorsQuery;
                descriptors2 = descriptorsDB;
            }
        }
    }
